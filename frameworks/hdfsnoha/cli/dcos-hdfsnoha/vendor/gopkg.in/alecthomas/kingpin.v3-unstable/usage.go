...
type UsageContext struct {
	// The text/hdfsnoha body to use.
	Template string
	// Indentation multiplier (defaults to 2 of omitted).
	Indent int
	// Width of wrap. Defaults wraps to the terminal.
	Width int
	// Funcs available in the hdfsnoha.
	Funcs hdfsnoha.FuncMap
	// Vars available in the hdfsnoha.
...
type hdfsnohaParseContext struct {
...
func (a *Application) UsageForContextWithTemplate(usageContext *UsageContext, parseContext *ParseContext) error { // nolint: gocyclo
	...
	t, err := hdfsnoha.New("usage").Funcs(funcs).Parse(tmpl)
	if err != nil {
		return err
	}
	appModel := a.Model()
